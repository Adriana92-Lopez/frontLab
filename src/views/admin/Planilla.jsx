...
import PlanillaModal from "../Modals/admin/PlanillaModal"

const Planilla = () => {
    const nameController = "Planilla"
    const [modalOpen, setModalOpen] = useState(false);
    const [opcion, setOpcion] = useState(0);
    const [contentInfor, setContentInfor] = useState(null);
    const [listPlanilla, setListPlanilla] = useState([]);

    const ListPlanilla = async () => {
        const response = await GetRoute(`${nameController}/getList`)
        setListPlanilla((response.length) ? response : [])
    },
    ...
    const updateState = async (item) => {
        let messageToast = 'Planilla ' + (item.estado === 1 ? 'activado' : 'desactivado') + ' correctamente.';
    ...
    return (
        <>
                    ...
                    <div className="col">
                        <Card className="shadow">
                            <CardHeader className="border-0">
                                <h3 className="mb-0">Administración de Planilla<Icon.PlusCircle onClick={() => toggleModal(null, 1)} className="ml-1 cursor-pointer text-green" size={25} style={{ cursor: 'pointer' }} /></h3>
            ...
            <PlanillaModal modalOpen={modalOpen} nameController={nameController} ListPlanilla={ListPlanilla} toggleModal={toggleModal} opcion={opcion} information={contentInfor} setInformation={setContentInfor} />
        </>
    );
};
