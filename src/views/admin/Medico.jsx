...
import MedicoModal from "../Modals/admin/MedicoModal"

const Medico = () => {
    const nameController = "Medico"
    ...
    const ListMedico = async () => {
        ...
        setListMedico((response.length) ? response : [])
    },
    ...
    const updateState = async (item) => {
        let messageToast = 'Medico ' + (item.estado === 1 ? 'activado' : 'desactivado') + ' correctamente.';
    ...
    return (
        <>
                    ...
                    <div className="col">
                        <Card className="shadow">
                            <CardHeader className="border-0">
                                <h3 className="mb-0">Administración de Medico<Icon.PlusCircle onClick={() => toggleModal(null, 1)} className="ml-1 cursor-pointer text-green" size={25} style={{ cursor: 'pointer' }} /></h3>
            ...
            <MedicoModal modalOpen={modalOpen} nameController={nameController} ListMedico={ListMedico} toggleModal={toggleModal} opcion={opcion} information={contentInfor} setInformation={setContentInfor} />
        </>
    );
};
