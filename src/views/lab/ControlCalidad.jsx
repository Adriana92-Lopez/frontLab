...
import ControlCalidadModal from "../Modals/admin/ControlCalidadModal"

const ControlCalidad = () => {
    const nameController = "ControlCalidad"
    const [modalOpen, setModalOpen] = useState(false);
    const [opcion, setOpcion] = useState(0);
    const [contentInfor, setContentInfor] = useState(null);
    const [listControlCalidad, setListControlCalidad] = useState([]);

    const ListControlCalidad = async () => {
        const response = await GetRoute(`${nameController}/getList`)
        setListControlCalidad((response.length) ? response : [])
    },
    ...
    const updateState = async (item) => {
        let messageToast = 'ControlCalidad ' + (item.estado === 1 ? 'activado' : 'desactivado') + ' correctamente.';
    ...
    return (
        <>
                    ...
                    <div className="col">
                        <Card className="shadow">
                            <CardHeader className="border-0">
                                <h3 className="mb-0">Administración de Control de Calidad
            ...
            <ControlCalidadModal modalOpen={modalOpen} nameController={nameController} ListControlCalidad={ListControlCalidad} toggleModal={toggleModal} opcion={opcion} information={contentInfor} setInformation={setContentInfor} />
        </>
    );
};
